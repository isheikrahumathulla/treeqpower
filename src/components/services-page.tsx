...
export function ServicesPage() {
  return <main>
    <section className="services-hero">
      ...
      <div className="zoho-shell services-hero-copy reveal"><p className="zoho-kicker">Our Services</p><h1>Integrated Engineering & Technical Services</h1><p>From engineering and design through testing, commissioning, maintenance, diagnostics and asset integrity, TreeQ Power provides practical technical services for electrical, electromechanical, industrial and infrastructure environments.</p><div><Button size="lg" asChild><a href="#service-directory">Explore Our Services<ArrowDown/></a></Button><Button size="lg" variant="outline" asChild><SiteLink to="/contact">Talk to Our Technical Team</SiteLink></Button></div></div>
    </section>

    <section className="services-intro reveal"><div className="zoho-shell services-intro-grid"><div><Heading eyebrow="Electrical and Engineering Services in UAE" title="Technical Services Across the Asset Lifecycle"/><div className="services-body"><p>TreeQ Power supports clients through multiple stages of the electrical and asset lifecycle — from engineering and design to installation support, testing, commissioning, troubleshooting, maintenance, diagnostics and asset integrity management.</p><p>Our service capabilities combine engineering knowledge with practical field execution, helping clients address technical requirements across electrical power systems, automation, instrumentation, MEP, testing and inspection.</p><p>Whether the requirement is a planned engineering assignment, specialist testing activity, equipment repair, shutdown support, fault investigation or ongoing asset management, our services can be structured around the technical and operational needs of the project.</p></div><Button variant="outline" asChild><SiteLink to="/about-us">About TreeQ Power<ArrowRight/></SiteLink></Button></div><figure><img src={serviceImages.intro} alt="Electrical engineering drawings and technical project planning" loading="lazy" width="900" height="700"/><figcaption>Engineering knowledge connected to practical field delivery.</figcaption></figure></div><div className="zoho-shell services-lifecycle" aria-label="Service lifecycle">{serviceLifecycle.map((item,index)=><div key={item}><span>{String(index+1).padStart(2,"0")}</span><strong>{item}</strong>{index<serviceLifecycle.length-1&&<ArrowRight/>}</div>)}</div></section>

    <section className="services-categories reveal"><div className="zoho-shell"><Heading eyebrow="Service Categories" title="Our Service Capabilities" intro="Four connected service groups help clients move from technical requirements to verified field outcomes and long-term asset support."/><div className="services-category-grid">{serviceCategories.map(({number,title,description,items,to,label,icon:Icon})=><SiteLink to={to} className="services-category-card" key={title}><div><span>{number}</span><Icon/></div><h3>{title}</h3><p>{description}</p><ul>{items.map(item=><li key={item}>{item}</li>)}</ul><strong>{label}<ArrowRight/></strong></SiteLink>)}</div></div></section>

    <section id="service-directory" className="services-directory reveal"><div className="zoho-shell"><Heading eyebrow="Complete Service Directory" title="Specialist Support for Systems, Sites & Assets" intro="Each service can be scoped around the equipment, operating environment, project stage and technical outcome required."/><div className="services-card-grid">{serviceCards.map(({title,description,areas,to,icon:Icon},index)=><SiteLink to={to} className="services-card" key={title}><div className="services-card-top"><span>{String(index+1).padStart(2,"0")}</span><Icon/></div><h3>{title}</h3><p>{description}</p><ul>{areas.map(area=><li key={area}><Check/>{area}</li>)}</ul><strong>View {title}<ArrowRight/></strong></SiteLink>)}</div></div></section>

    <section className="services-integrity reveal"><div className="services-integrity-image"><img src={serviceImages.integrity} alt="Industrial plant equipment supported by asset integrity services" loading="lazy" width="1400" height="1000"/></div><div className="services-integrity-copy"><ShieldCheck/><Heading eyebrow="Asset Integrity" title="Risk-Based Asset Integrity Services"/><p>TreeQ Power provides structured asset integrity services to help organizations understand equipment condition, identify risk, prioritize inspection activities and support lifecycle decisions.</p><p>Our approach can combine asset information, inspection history, failure mechanisms, condition assessment, criticality and operational considerations to establish practical inspection and integrity strategies.</p><Button variant="outline" asChild><SiteLink to="/our-services/asset-integrity">Explore Asset Integrity Services<ArrowRight/></SiteLink></Button></div></section>

    <section className="services-integrity-cards reveal"><div className="zoho-shell"><div>{assetIntegrityServices.map((item,index)=><SiteLink to={item.to} key={item.title}><span>{String(index+1).padStart(2,"0")}</span><h3>{item.title}</h3><p>{item.text}</p><strong>Explore service<ArrowRight/></strong></SiteLink>)}</div></div></section>
    ...
    <section className="services-industries reveal"><div className="zoho-shell"><div className="services-industries-head"><Heading eyebrow="Industries Served" title="Services Across Critical Industries"/><Button variant="outline" asChild><SiteLink to="/industries">Explore All Industries<ArrowRight/></SiteLink></Button></div><div className="services-industry-grid">{serviceIndustries.map(item=><SiteLink to={item.to} key={item.title}><img src={item.image} alt={item.alt} loading="lazy" width="650" height="760"/><div><h3>{item.title}</h3><p>{item.text}</p><ArrowRight/></div></SiteLink>)}</div></div></section>
