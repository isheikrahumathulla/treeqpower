...
import { groupPages } from "@/lib/service-groups";
import {
...
export function ServicesPage() {
  return <main>
    ...
    <section className="services-categories reveal"><div className="zoho-shell"><Heading eyebrow="Service Categories" title="Our Service Capabilities" intro="Four connected service groups help clients move from technical requirements to verified field outcomes and long-term asset support."/><div className="services-category-grid">{serviceCategories.map(({number,title,description,items,to,label,icon:Icon})=>{const group=groupPages[to];return <SiteLink to={to} className="services-category-card" key={title}>{group&&<><img className="services-category-bg" src={group.image} alt="" loading="lazy" width="1200" height="900"/><span className="services-category-veil" aria-hidden="true"/></>}<div className="services-category-body"><div><span>{number}</span><Icon/></div><h3>{title}</h3><p>{description}</p><ul>{items.map(item=><li key={item}>{item}</li>)}</ul><strong className="services-category-cta">{label}</strong></div></SiteLink>;})}</div></div></section>

    <section id="service-directory" className="services-directory reveal"><div className="zoho-shell"><Heading eyebrow="Complete Service Directory" title="Specialist Support for Systems, Sites & Assets" intro="Each service can be scoped around the equipment, operating environment, project stage and technical outcome required."/><div className="services-card-grid">{serviceCards.map(({title,description,areas,to,icon:Icon},index)=><SiteLink to={to} className="services-card" key={title}><div className="services-card-top"><span>{String(index+1).padStart(2,"0")}</span><Icon/></div><h3>{title}</h3><p>{description}</p><ul>{areas.map(area=><li key={area}><Check/>{area}</li>)}</ul><strong>View {title}</strong></SiteLink>)}</div></div></section>

    <section className="services-integrity reveal"><div className="services-integrity-image"><img src={serviceImages.integrity} alt="Industrial plant equipment supported by asset integrity services" loading="lazy" width="1400" height="1000"/></div><div className="services-integrity-copy"><ShieldCheck/><Heading eyebrow="Asset Integrity" title="Risk-Based Asset Integrity Services"/><p>TreeQ Power provides structured asset integrity services to help organizations understand equipment condition, identify risk, prioritize inspection activities and support lifecycle decisions.</p><p>Our approach can combine asset information, inspection history, failure mechanisms, condition assessment, criticality and operational considerations to establish practical inspection and integrity strategies.</p><Button variant="outline" asChild><SiteLink to="/our-services/asset-integrity"><strong>EXPLORE</strong></SiteLink></Button></div></section>

    <section className="services-integrity-cards reveal"><div className="zoho-shell"><div>{assetIntegrityServices.map((item,index)=><SiteLink to={item.to} key={item.title}><span>{String(index+1).padStart(2,"0")}</span><h3>{item.title}</h3><p>{item.text}</p><strong>Explore service</strong></SiteLink>)}</div></div></section>
