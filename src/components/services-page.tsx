...
export function ServicesPage() {
  return <main>
    ...
    <section className="services-categories reveal"><div className="zoho-shell"><Heading eyebrow="Service Categories" title="Our Service Capabilities" intro="Four connected service groups help clients move from technical requirements to verified field outcomes and long-term asset support."/><div className="services-category-grid">{serviceCategories.map(({number,title,description,to,label,icon:Icon})=><SiteLink to={to} className="services-category-card" key={title}><div className="services-category-body"><div><span>{number}</span><Icon/></div><h3>{title}</h3><p>{description}</p><strong className="services-category-cta">{label}</strong></div></SiteLink>)}</div></div></section>
