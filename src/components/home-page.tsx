...
export function HomePage() {
  return <main>
    ...
    <section className="home-catalog reveal"><div className="zoho-shell"><SectionHeading eyebrow="Our Services" title="Technical Services for Electrical, Industrial & Critical Assets"/><div className="home-scroll-grid home-services-carousel" aria-label="Our services" tabIndex={0}><div className="home-scroll-track">{services.map(item => <ImageCard item={item} key={item.title}/>)}{services.map(item => <div aria-hidden="true" key={`duplicate-${item.title}`}><ImageCard item={item}/></div>)}</div></div><Button variant="outline" asChild><SiteLink to="/services">View All Services<ArrowRight/></SiteLink></Button></div></section>
